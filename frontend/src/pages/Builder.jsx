...
const newSkillGroup = () => ({ id: uid(), category: 'Frontend', items: [] });

function Input({ label, value, onChange, type = 'text', placeholder = '' }) {
  return <label className="builder-field"><span>{label}</span><input type={type} value={value || ''} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} /></label>;
}

function Textarea({ label, value, onChange, placeholder = '', rows = 4 }) {
  return <label className="builder-field"><span>{label}</span><textarea rows={rows} value={value || ''} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} /></label>;
}
...
export default function Builder() {
  ...
  useEffect(() => {
    ...
    try {
      const parsed = currentResume.resumeData ? JSON.parse(currentResume.resumeData) : {};
      setData({ ...defaultData, ...parsed, personal: { ...defaultData.personal, ...(parsed.personal || {}) }, links: { ...defaultData.links, ...(parsed.links || {}) } });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const saved = await updateResume(id, { title: data.personal.fullName || 'Untitled Resume', template, resumeData: data });
  ...
  const updateArray = (section, index, patch) => setData((d) => ({ ...d, [section]: d[section].map((item, i) => i === index ? { ...item, ...patch } : item) }));
  const removeArray = (section, index) => setData((d) => ({ ...d, [section]: d[section].filter((_, i) => i !== index) }));
  const moveArray = (section, index, direction) => setData((d) => {
    const arr = [...d[section]]; const next = index + direction;
    if (next < 0 || next >= arr.length) return d;
    [arr[index], arr[next]] = [arr[next], arr[index]];
    return { ...d, [section]: arr };
  });
  const addArray = (section, item) => setData((d) => ({ ...d, [section]: [...d[section], item()] }));

  const renderPreview = useMemo(() => {
    if (template === 'Executive') return <Executive data={data} />;
    if (template === 'ModernCreative') return <ModernCreative data={data} />;
  ...
  return <div className="builder-shell">
    <header className="builder-toolbar">
      ...
      <div className="toolbar-center"><button className={activeTab === 'edit' ? 'active' : ''} onClick={() => setActiveTab('edit')}>Edit</button><button className={activeTab === 'preview' ? 'active' : ''} onClick={() => setActiveTab('preview')}><Eye size={15} /> Preview</button></div>
    ...
    <div className="builder-workspace">
      <aside className={`builder-editor ${activeTab === 'edit' ? 'mobile-visible' : ''}`}>
        <div className="editor-inner">
          <div className="editor-intro"><span>Resume details</span><h1>Build your resume</h1><p>Everything you change here updates the preview instantly.</p></div>
          ...
          <SectionCard title="Skills" count={data.skills.length} open={open.skills} onToggle={() => setOpen(o => ({ ...o, skills: !o.skills }))} onAdd={() => addArray('skills', newSkillGroup)}>
            {data.skills.map((s, i) => <ItemCard key={s.id || i} title={s.category || 'Skill category'} first={i === 0} last={i === data.skills.length - 1} onDelete={() => removeArray('skills', i)} onMoveUp={() => moveArray('skills', i, -1)} onMoveDown={() => moveArray('skills', i, 1)}>
              <Input label="Category" value={s.category} onChange={(v) => updateArray('skills', i, { category: v })} placeholder="Frontend" /><Input label="Skills (comma separated)" value={(s.items || []).join(', ')} onChange={(v) => updateArray('skills', i, { items: v.split(',').map(x => x.trim()).filter(Boolean) })} placeholder="HTML, CSS, JavaScript, React.js" />
            </ItemCard>)}
          </SectionCard>
